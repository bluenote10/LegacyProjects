...
FourierBoth(origsample)

filter1DFT = multlist(GenerateGaussSample(9, 4, 2), +1)
filter1DFT = MirrorSample(filter1DFT, True, +1)
filter1coef = FourierBoth(filter1DFT)

filter1out = ApplyFilter(origsample, filter1coef)
FourierBoth(filter1out)

filter2DFT = multlist(GenerateGaussSample(9, 4, 2), -1j)
filter2DFT = MirrorSample(filter2DFT, True, -1)
filter2coef = FourierBoth(filter2DFT)

filter2out = ApplyFilter(origsample, filter2coef)
FourierBoth(filter2out)

allout = []
for i in range(len(filter1out)):
    allout.append(filter1out[i] + filter2out[i])
FourierBoth(allout)
